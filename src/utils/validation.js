...
const validateInputData = (req)=>{
    ...
    if(firstName && (firstName.length < 4 || firstName.length > 15)){
        throw createError(400, "Firstname length should be between 4 to 15...");
    }
    if(lastName && (lastName.length < 4 || lastName.length > 15)){
        throw createError(400, "Lastname length should be between 4 to 15...");
    }
    if(email){
        if(!validator.isEmail(email)) throw createError(400, "Enter valid email!");
    }
    if(password){
    ...
    if(age){
        if(age < 16) throw createError(400, "You are underage!");
...
const validateEditData = (req)=>{
    ...
    if(!isEditAllowed) throw createError(400, "Edit not allowed!");
};
